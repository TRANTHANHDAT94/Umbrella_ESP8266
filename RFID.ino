...
MFRC522 mfrc522_rfid_umbrella(SS_PIN, RST_PIN);  //--> Create MFRC522 instance.
//Configure RFID Felica Card
#define SS_PIN D4  //--> SDA / SS is connected to pinout D4
#define RST_PIN D3  //--> RST is connected to pinout D1
MFRC522 mfrc522_rfid_felica(SS_PIN, RST_PIN);  //--> Create MFRC522 instance.
//mfrc522_rfid_felica will be use to check request from client 
...
void setup() {
  ...
  mfrc522_rfid_umbrella.PCD_Init(); //--> Init MFRC522 card
  mfrc522_rfid_felica.PCD_Init(); //--> Init MFRC522 card
  delay(500);
...
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------//
//ALgorithm will be showed here...(updating)
//----------------------------------------Procedure for reading and obtaining a UID from a card or keychain---------------------------------------------------------------------------------//
int getid() {
  if (!mfrc522_rfid_umbrella.PICC_IsNewCardPresent()) {
    return 0;
  }
  if (!mfrc522_rfid_umbrella.PICC_ReadCardSerial()) {
    return 0;
  }
  Serial.print("THE UID OF THE SCANNED CARD IS : ");
  for (int i = 0; i < 4; i++) {
    readcard[i] = mfrc522_rfid_umbrella.uid.uidByte[i]; //storing the UID of the tag in readcard
    array_to_string(readcard, 4, str);
    StrUID = str;
  }
  mfrc522_rfid_umbrella.PICC_HaltA();
  return 1;
}
//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------//
